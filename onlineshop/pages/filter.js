...
const Drop = ({ title, list }) => {
  ...
  return (
    <div className="border-2 p-2">
      <div
        onClick={() => setHide((s) => !s)}
        className="flex items-center justy-between"
      >
        <h1>{title}</h1>
        {hide ? (
          <ArrowNarrowUpIcon className="w-3 h-3 text-primary" />
        ) : (
          <ArrowNarrowDownIcon className="w-3 h-3 text-primary" />
        )}
      </div>
      {hide ? (
        <div>
